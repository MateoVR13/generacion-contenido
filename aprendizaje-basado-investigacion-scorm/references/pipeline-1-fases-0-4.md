...
> Nota de nomenclatura: en ABI los nombres de los **momentos** son fijos. Para evitar choque con la palabra
> "fase", en prosa puedes referir las fases institucionales del proceso UA como **"Etapa 0–4"**; los nombres de
> fase del pipeline (F0–F10) se mantienen tal cual.
...
## Fase 2 — Planeación didáctica (diseño inverso, ABI por momentos)
- **Hacer:** diseñar la planeación virtual desde el RA y la configuración por créditos. Justificar ABI.
  **Planear los 4 momentos fijos de ABI**
  (`["Problematización", "Desarrollo teórico", "Metodología e implementación", "Resultados y conclusiones"]`)
  con su tabla de diseño (Momento | RA | propósito del momento | preguntas/problemas de investigación).
  En ABI **no se propone un número de momentos**: siempre son 4 y en ese orden; lo que se valida es el
  propósito, las preguntas/problemas de investigación y la cobertura de cada momento. Definir ruta de
  navegación (escenario inicial = líneas de investigación + preguntas/problemas), actividades
...
- **Entregables (`fase-2-planeacion-didactica.md`):** configuración didáctica; justificación ABI; RA eje; ruta
  del estudiante; tabla de los 4 momentos ABI; distribución de horas; recursos proyectados;
  **matriz de material complementario propuesto + cobertura por momento (PROPUESTA, a validar)**.
- **Estado:** `momentos[]{titulo, ra, propositoEtapa, preguntasOrientadoras[], subtemas[]}` (4 elementos fijos),
...
- **Hacer:** estrategia evaluativa alineada al RA y ABI, cualitativa (`lineamientos-ua.md`). Mapa de
...
- **TALLERES por escenario — REDACTAR el propuesto por momento (va al instrumento).** Para cada momento, propón
