...
Generated asignaturas are 100% virtual and must use only ABI - Aprendizaje Basado en Investigación. Do not choose or mention other active methodologies in generated JSON.

The ABI enfoque is research-driven: the theoretical document guides the learner to formulate and develop an investigation. The initial scenario presents the disciplinary research lines and example research questions/problems; the knowledge document then explains the importance of theoretical/contextual frameworks, the discipline's research methodologies, and the presentation of results, analysis, and conclusions. Keep the emphasis on problem statement, theoretical framework, methodological design, and results.

Official ABI moments:
...
Use diagnostic, formative, and final evaluation inside the ABI route; those evaluation moments do not replace the official methodology moments.

## Four Content Package Structure

Every new asignatura generation must produce exactly four independent JSON files, one per ABI moment:

1. `<slug-asignatura>-contenido-01.json` — Problematización
2. `<slug-asignatura>-contenido-02.json` — Desarrollo teórico
3. `<slug-asignatura>-contenido-03.json` — Metodología e implementación
4. `<slug-asignatura>-contenido-04.json` — Resultados y conclusiones

Each JSON file must include:

- `subject.contentNumber` from `01` to `04`.
- `subject.contentTotal: "04"`.
- `subject.methodology.code: "ABI"`.
...
- Run content uniqueness validation before finalizing. If Node.js is available, run `node aprendizaje-basado-investigacion-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until repeated learner-facing content is eliminated.
