...
actualizar `estado-proyecto.json`: ajustes a propósitos/preguntas-problemas de los 4 momentos, actividades del
estudiante aprobadas, recursos (incluidos los de **simulación/laboratorio si la asignatura es práctica**),
sugerencias y feedback. A partir de aquí todo se genera con los valores VALIDADOS. (En ABI los momentos son 4
fijos: no se valida un número de momentos, solo su contenido.)

---

## Fase 5 — Diseño tecnopedagógico y disciplinar (Documento de Saberes)
- **Hacer:** producir el Documento de Saberes ABI (4 momentos validados, extensión por créditos).
- **Salida OBLIGATORIA: los 4 archivos JSON finales** `<slug>-contenido-NN.json` (uno por momento, modelo ABI
  = 4 momentos = 4 archivos: `01` Problematización, `02` Desarrollo teórico, `03` Metodología e
  implementación, `04` Resultados y conclusiones), conformes al contrato de la plantilla SCORM/PDF
  (`references/json-contract.md`). Estos JSON son cargables directamente en la plantilla; **no basta con dejar
  un esquema en Markdown**. El esquema de contenido es solo un paso intermedio interno: la Fase 5 NO se
  considera cerrada hasta que existan los 4 JSON parseables y válidos.
- **Usa las references de ESTA misma skill** (`aprendizaje-basado-investigacion-scorm`) para el contrato y las
  reglas de componentes (3 bloques teóricos por sección, carrusel/flashcards/acordión, steppers en LaTeX,
  charts con description+note): `references/json-contract.md`, `references/component-selection.md`,
  `references/instructional-content-requirements.md`, `references/math-latex-validation.md`,
  `references/content-uniqueness-validation.md`. Aplica los parámetros validados (4 momentos,
  propósito/preguntas-problemas de investigación por momento, RA, extensión por créditos, decisiones del
  docente) con `unitLabel: "Momento"` y `contentTotal: "04"`. No delega a otra skill: esta skill es el motor.
- Cada concepto central: definición, contexto teórico, principios, relevancia, aplicación en ABI, ejemplos
...
- **Talleres por escenario:** cada momento lleva su taller (aprobado en P1) redactado completo dentro de su
  ...
  retroalimentación). La **sumativa integradora** (todos los momentos/RA, última semana) en el último momento
  (Resultados y conclusiones) o `evaluacion/sumativa-<slug>.json`.
...
- **Reutiliza `moodle-content`**: genera el JSON de los componentes Moodle (bienvenida, página por momento,
  cierre) que el renderer convierte en HTML pegable. Pasa los 4 momentos ABI (ABI: una página por momento) con
  `unitLabel: "Momento"`. El archivo de Moodle se nombra `<slug>-ABI.moodle.json`.
- **Entregables:** JSON de moodle-content (`<slug>-ABI.moodle.json`) + `fase-7-montaje-lms.md` (plan de aula,
...
- Fase 5 usa las references de contenido de esta misma skill; Fase 7 delega a moodle-content; no reproducir su
