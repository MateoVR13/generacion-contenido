...
1. Convert the user request and any provided syllabus into a package of **four** reusable `subject` JSON files for SCORM/PDF, one file per Aprendizaje Basado en Investigación moment.
...
5. Use only the files inside this `aprendizaje-basado-investigacion-scorm/` folder as the skill's reusable reference base; do not depend on `../requisitos-contenido/` or external attachments unless the user provides new source material in the current task.
...
8. For new asignatura generation, write exactly four artifacts in the current project root using this naming pattern: `<slug-asignatura>-contenido-01.json` through `<slug-asignatura>-contenido-04.json`. Each numbered file develops the matching Aprendizaje Basado en Investigación moment (`01` = Problematización, `02` = Desarrollo teórico, `03` = Metodología e implementación, `04` = Resultados y conclusiones). Do not create, overwrite, or replace `content.json` for generated asignatura content.
...
11. Use only **ABI - Aprendizaje Basado en Investigación** as methodology for generated virtual asignaturas. Do not use guided, project, challenge, case, or any other active methodology unless the app and institutional rules are explicitly changed later.
12. Each of the four JSON files must contain exactly six SCORM sections and exactly six PDF sections: one introductory section plus five content sections. The five internal sections are NOT the methodology moments; each file as a whole develops one ABI moment.
...
18. Run a dedicated content-uniqueness validation pass before finalizing. If Node.js is available, run `node aprendizaje-basado-investigacion-scorm/scripts/validate_content_package.js <generated-json-files>`. Treat repeated learner-facing paragraphs, bullets, examples, exercises, feedback, code scaffolds, or visual prompts as blocking defects and rewrite them conceptually.
...
- Treat every asignatura as 100% virtual and generated as four content JSON files, one per Aprendizaje Basado en Investigación moment. Do not generate a single all-course JSON for a new asignatura.
- Every generated JSON must represent one content number (`subject.contentNumber`: `01` to `04`) and should include `subject.contentTotal: "04"`. Content `01` develops the moment Problematización, `02` Desarrollo teórico, `03` Metodología e implementación, and `04` Resultados y conclusiones.
...
- Use `subject.methodology.code: "ABI"`, `subject.methodology.name: "Aprendizaje Basado en Investigación"`, and `subject.methodology.moments: ["Problematización", "Desarrollo teórico", "Metodología e implementación", "Resultados y conclusiones"]` in every JSON.
...
- For methodology planning tables, routes, and `subject.methodology.moments`, use the official Aprendizaje Basado en Investigación moment names as the labels. Write `Problematización`, `Desarrollo teórico`, `Metodología e implementación`, `Resultados y conclusiones`; do not relabel them as `Fase`, `Etapa`, or `Tema o etapa`, and do not invent other moment names.
- The asignatura presentation must introduce the discipline, purpose, scope, the Aprendizaje Basado en Investigación route, expected student work, and professional/academic value. Frame the opening around the research enfoque: present the disciplinary research lines, example research questions/problems, and how the theoretical document prepares the learner to formulate and develop an investigation through its moments (Problematización, Desarrollo teórico, Metodología e implementación, Resultados y conclusiones). Do not use the opening to list isolated technical topics or project slogans.
...
- Check every generated file has `subject.contentNumber`, `subject.contentTotal: "04"`, and consistent title/metadata for that content, and that the file develops its matching ABI moment (`01` Problematización, `02` Desarrollo teórico, `03` Metodología e implementación, `04` Resultados y conclusiones).
...
- Check methodology moments and tables use the official ABI moment names (`Problematización`, `Desarrollo teórico`, `Metodología e implementación`, `Resultados y conclusiones`), never `Fase`, `Etapa`, or `Tema o etapa`.
- Check the only methodology is ABI - Aprendizaje Basado en Investigación; do not use any other methodology name, code, moments, labels, or structure.
...
- Run the content uniqueness validation pass from `references/content-uniqueness-validation.md`; when possible execute `node aprendizaje-basado-investigacion-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until it passes.
