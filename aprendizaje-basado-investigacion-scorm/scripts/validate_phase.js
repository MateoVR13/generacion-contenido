#!/usr/bin/env node
"use strict";
// Valida el estado-proyecto.json contra los lineamientos UA para ABI.
...
// ABI: 4 momentos fijos, en este orden.
const MOMENTOS_ABI = ["Problematización", "Desarrollo teórico", "Metodología e implementación", "Resultados y conclusiones"];
...
if (st.metodologia !== "ABI") errors.push(`metodologia debe ser "ABI", es "${st.metodologia}"`);
...
// ABI: 4 momentos fijos, no variable. Se planean en F2.
if (fase >= 2) {
  if (!Array.isArray(st.momentos)) {
    errors.push("momentos no definido (ABI: deben planearse los 4 momentos fijos en F2)");
  } else if (st.momentos.length !== 4) {
    errors.push(`ABI exige exactamente 4 momentos fijos; momentos.length=${st.momentos.length}`);
  } else {
    st.momentos.forEach((m, i) => {
      if (m && m.titulo && m.titulo !== MOMENTOS_ABI[i])
        warnings.push(`momentos[${i}].titulo="${m.titulo}"; oficial ABI = "${MOMENTOS_ABI[i]}"`);
    });
  }
}
...
if (st.pipeline2Completo === true) {
  ...
  if (dsCount === 0)
    ...
    warnings.push(`Documento de Saberes: ${dsCount} JSON encontrados, se esperaban 4 (uno por momento ABI).`);
  ...
  if (!hasMoodle)
    errors.push("Pipeline 2 marcado completo pero NO hay JSON de Moodle (fase-7-montaje-lms/<slug>-ABI.moodle.json).");
