...
Generated asignaturas are 100% virtual and must use only ABC - Aprendizaje Basado en Casos. Do not choose or mention other active methodologies in generated JSON.

Official ABC learning moments (each one is developed by one JSON file, in this order):
...
"methodology": {
  "code": "ABC",
  "name": "Aprendizaje Basado en Casos",
  "rationale": "",
  "moments": ["Presentación y análisis del caso", "Análisis de pregunta orientadora o micro caso 1", "Análisis de pregunta orientadora o micro caso 2", "Conclusiones"]
}
```

The didactic focus is the analysis of a real disciplinary CASE: the initial scenario presents the case, and the documento de saberes develops the analysis of the case and of guiding questions / micro-cases, providing the theoretical bases to interpret them and culminating in conclusions. Emphasize critical analysis of real situations, guiding questions, and micro cases. Use diagnostic, formative, and final evaluation inside the case-based route; those evaluation moments do not replace the official learning moments.

## Four Content Package Structure

Every new asignatura generation must produce exactly four independent JSON files, one per Aprendizaje Basado en Casos learning moment:

1. `<slug-asignatura>-contenido-01.json` — moment "Presentación y análisis del caso".
2. `<slug-asignatura>-contenido-02.json` — moment "Análisis de pregunta orientadora o micro caso 1".
3. `<slug-asignatura>-contenido-03.json` — moment "Análisis de pregunta orientadora o micro caso 2".
4. `<slug-asignatura>-contenido-04.json` — moment "Conclusiones".

Each JSON file must include:

- `subject.contentNumber` from `01` to `04`.
- `subject.contentTotal: "04"`.
- `subject.methodology.code: "ABC"`.
...
The four files are the methodology's learning moments; the five internal thematic sections of each file are NOT the moments, they develop the moment of that file.
...
- Run content uniqueness validation before finalizing. If Node.js is available, run `node aprendizaje-basado-casos-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until repeated learner-facing content is eliminated.
