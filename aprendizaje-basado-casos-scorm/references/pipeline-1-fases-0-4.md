...
## Fase 2 — Planeación didáctica (diseño inverso, ABC)
- **Hacer:** diseñar la planeación virtual desde el RA y la configuración por créditos. Justificar ABC.
  ABC se organiza por **4 momentos FIJOS** (no se propone un N variable): `["Presentación y análisis del
  caso", "Análisis de pregunta orientadora o micro caso 1", "Análisis de pregunta orientadora o micro caso
  2", "Conclusiones"]`. Construir la **tabla de diseño por momento** (Momento | RA | propósito del momento |
  preguntas orientadoras). **Definir el CASO real disciplinar** (escenario inicial: caso real + micro-casos /
  preguntas orientadoras) que articula los 4 momentos: contexto, datos, actores y problema del caso, más los
  dos micro-casos / preguntas orientadoras (momentos 2 y 3). Definir ruta de navegación, actividades
  sincrónicas/asincrónicas, distribución de horas, documento y recursos proyectados, secuencia evaluativa.
  **No inventes datos del caso ni micro-casos que el syllabus/fuentes no respalden**; lo que no esté soportado,
  decláralo como brecha.
- **PROPONER LOS SUBTEMAS RELEVANTES por momento (solo listado, a validar).** Para cada uno de los 4 momentos,
...
- **Entregables (`fase-2-planeacion-didactica.md`):** configuración didáctica; justificación ABC; RA eje;
  **definición del caso real + micro-casos / preguntas orientadoras**; ruta del estudiante; tabla de los 4
  momentos ABC (PROPUESTA, a validar); distribución de horas; recursos proyectados; **matriz de material
  complementario propuesto + cobertura por momento (PROPUESTA, a validar)**.
- **Estado:** `caso{titulo, contexto, microCasos[]}`,
  `momentos[]{n, titulo, ra, propositoEtapa, preguntasOrientadoras[], subtemas[]}` (los 4 momentos fijos),
...
- **Hacer:** estrategia evaluativa alineada al RA y ABC, cualitativa (`lineamientos-ua.md`). Mapa de
