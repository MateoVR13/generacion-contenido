...
Use this reference before writing or finalizing any package of four JSON files. Passing the structural contract is not enough: every scenario/content (Aprendizaje Basado en Casos learning moment) and every thematic section must be academically unique, clearly aligned with learning outcomes, and free of repeated boilerplate.
...
For each `contenido-01` through `contenido-04` (the four Aprendizaje Basado en Casos learning moments), and each `seccion-1` through `seccion-5`, define:
...
Generate the JSON only after the blueprint shows 20 distinct thematic sections. If two rows could swap titles and still make sense, they are too generic.
...
```bash
node aprendizaje-basado-casos-scorm/scripts/validate_content_package.js <slug-asignatura>-contenido-01.json <slug-asignatura>-contenido-02.json <slug-asignatura>-contenido-03.json <slug-asignatura>-contenido-04.json
```
