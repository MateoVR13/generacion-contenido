...
description: Virtualiza asignaturas 100% virtuales con metodología Aprendizaje Basado en Casos (ABC) siguiendo el proceso oficial UA de 11 fases (0–10) en 2 pipelines con gate de validación docente, Y genera los JSON de contenido SCORM/PDF. Úsala para (a) ejecutar el Pipeline 1 (Fases 0–4) que produce el INSTRUMENTO DE VALIDACIÓN DOCENTE a partir de un syllabus, y (b) ejecutar el Pipeline 2 (Fases 5–10) que, con el instrumento validado, genera el Documento de Saberes (los 4 JSON `subject`-based para la plantilla SCORM/PDF), los recursos e-learning y sus prompts, el montaje LMS y los entregables de QA/implementación/seguimiento. ABC usa 4 MOMENTOS FIJOS (Presentación y análisis del caso · Análisis de pregunta orientadora o micro caso 1 · Análisis de pregunta orientadora o micro caso 2 · Conclusiones): el estudiante analiza un CASO real disciplinar y el Documento de Saberes desarrolla el análisis del caso y de micro-casos/preguntas orientadoras con las bases teóricas para interpretarlos, culminando en conclusiones. Extensión por créditos (30–55 págs), evaluación cualitativa, similitud ≤30% y APA 7. La Fase 5 aplica el contrato de componentes de esta misma skill: selección experta por disciplina, secuencia theory-first (≥3 theory-block por sección temática), carrusel/flashcards/acordeón obligatorios, steppers en matemáticas, Python para resolver ejercicios cuantitativos, charts con hilo conductor + nota, LaTeX validado, banner saberes-link, PDF editorial por facultad.
---

# Virtualización ABC + Contenido SCORM/PDF

> Esta skill hace **dos cosas**: (1) **orquesta** el proceso oficial de virtualización UA (11 fases, 2 pipelines, gate docente) y (2) en la **Fase 5** **genera** los JSON de contenido SCORM/PDF. La parte de orquestación está aquí abajo en "Proceso de virtualización"; la parte de generación es el "Core Workflow" y las reglas de contenido (más abajo), que se aplican **dentro de la Fase 5** y NO cambian.
...
SYLLABUS (Excel FO_03 / PDF / texto)
   ...
   ▼  [ GATE HUMANO: el docente valida caso/momentos/actividades/recursos/evaluación ]
...
3. **Lee solo lo de la fase en curso** (token-eficiencia): siempre `references/lineamientos-ua.md` + `references/proceso-fases-ua.md` + `references/metodologia-abc.md`; Pipeline 1 → `references/pipeline-1-fases-0-4.md` + `references/instrumento-docente.md`; Pipeline 2 → `references/pipeline-2-fases-5-10.md` + `references/pipeline-documento-saberes.md` + `references/fase-6-prompts.md`.
...
7. **Pipeline 2 arranca** con el instrumento respondido: ingiere las decisiones del docente al estado y genera todo. La **Fase 5** produce los **4 JSON del Documento de Saberes** (uno por momento) siguiendo el **Core Workflow** y las reglas de contenido de abajo (esta misma skill es el motor de contenido — no delega a otra). La **Fase 7** usa `moodle-content`.
8. **Cierre del Pipeline 2:** deben existir y validar los 4 JSON `<slug>-contenido-NN.json` (Documento de Saberes, uno por momento) y el `<slug>-ABC.moodle.json` (Moodle).

> **Metodología ABC = 4 momentos FIJOS, un archivo por momento.** A diferencia de AG (N temas variables), ABC tiene exactamente 4 momentos en orden fijo: `01` = "Presentación y análisis del caso", `02` = "Análisis de pregunta orientadora o micro caso 1", `03` = "Análisis de pregunta orientadora o micro caso 2", `04` = "Conclusiones". El docente valida el CASO, los micro-casos/preguntas orientadoras, propósitos, preguntas y subtemas por momento, NO el número de momentos. `unitLabel` = "Momento", `contentTotal` = "04". El archivo de Moodle se nombra `<slug>-ABC.moodle.json`.
