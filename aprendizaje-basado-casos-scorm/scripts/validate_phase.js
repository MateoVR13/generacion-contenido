#!/usr/bin/env node
"use strict";
// Valida el estado-proyecto.json contra los lineamientos UA para ABC.
...
// ABC: 4 momentos fijos (no variables).
const N_MOMENTOS = 4;
...
if (st.metodologia !== "ABC") errors.push(`metodologia debe ser "ABC", es "${st.metodologia}"`);
...
// ABC: 4 momentos FIJOS, no variable. El array de momentos debe tener exactamente 4 elementos desde Fase 2.
if (fase >= 2) {
  if (!Array.isArray(st.momentos)) {
    errors.push("momentos no definido (ABC: deben planearse los 4 momentos fijos en F2)");
  } else if (st.momentos.length !== N_MOMENTOS) {
    errors.push(`ABC tiene ${N_MOMENTOS} momentos fijos; momentos.length=${st.momentos.length}`);
  }
}
...
if (st.pipeline2Completo === true) {
  ...
  if (dsCount === 0)
    ...
    errors.push(`Documento de Saberes: ${dsCount} JSON encontrados, se esperaban ${N_MOMENTOS} (ABC = 4 momentos = 4 archivos).`);
  ...
  if (!hasMoodle)
    errors.push("Pipeline 2 marcado completo pero NO hay JSON de Moodle (fase-7-montaje-lms/<slug>-ABC.moodle.json).");
