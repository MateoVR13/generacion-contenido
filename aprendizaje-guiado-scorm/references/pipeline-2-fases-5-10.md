...
- **El branch PDF es el DOCUMENTO DE SABERES con narrativa, no un volcado de bullets** (ver `json-contract.md` →
  "Narrativa guiada"). Cada sección PDF: párrafo-puente de apertura que conecta con el tema anterior →
  teoría como prosa guiada con citas APA → figura/`image` con pie y lectura → ejemplo/`code` contextualizado
...
- **Código en cada tema (asignaturas de programación/cuantitativas):** incluye al menos un ejemplo de código
  (componente `code`, p. ej. Python) contextualizado al saber del tema, y al menos un ejercicio `kind:codigo`.
  En temas conceptuales sin código natural, prioriza pseudocódigo/diagrama pero mantén un ejercicio aplicado.
