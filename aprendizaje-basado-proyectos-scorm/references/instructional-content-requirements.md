...
Generated asignaturas are 100% virtual and must use only ABPr - Aprendizaje Basado en Proyectos. Do not choose or mention other active methodologies in generated JSON.

Official ABPr moments (one JSON file per moment; content NN develops moment NN):

- Diagnóstico Inicial (contenido-01)
- Análisis y Diseño (contenido-02)
- Conclusiones (contenido-03)
...
"methodology": {
  ...
  "rationale": "",
  "moments": ["Diagnóstico Inicial", "Análisis y Diseño", "Conclusiones"]
}
```

Use diagnostic, formative, and final evaluation inside the ABPr route; those evaluation moments do not replace the official moments.

The didactic focus: the theoretical content prepares the learner to design and develop a PROJECT. The initial scenario poses a real problem with a guiding question ("pregunta direccionadora"); the documento de saberes supplies the conceptual bases to identify and implement transformation actions, analyze the problem, plan and design the solution, and socialize the project product, with emphasis on collaborative teamwork in equipos.

## Three Content Package Structure

Every new asignatura generation must produce exactly three independent JSON files, one per ABPr learning moment:

1. `<slug-asignatura>-contenido-01.json` — Diagnóstico Inicial
2. `<slug-asignatura>-contenido-02.json` — Análisis y Diseño
3. `<slug-asignatura>-contenido-03.json` — Conclusiones

Each JSON file must include:

- `subject.contentNumber` from `01` to `03`.
- `subject.contentTotal: "03"`.
- `subject.methodology.code: "ABPr"`.
...
The user-provided PDF extension applies to each PDF file. If the user requests "mínimo 12 páginas", each of the three `pdf` branches must support 12 printable pages.

Across the full package, the 15 thematic sections must be different. A package that repeats the same theoretical shell, practice steps, exercise pattern, expected answer, feedback, visual prompt, or code scaffold with only topic names changed does not meet academic quality requirements, even if the JSON structure is valid.
...
Before drafting text, apply `content-uniqueness-validation.md`: plan the 3 x 5 package as a diversity matrix and ensure every thematic section has unique RA evidence, example/case, visual artifact, code or trace task, misconception, guided practice, feedback, and transfer.
...
- Run content uniqueness validation before finalizing. If Node.js is available, run `node aprendizaje-basado-proyectos-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until repeated learner-facing content is eliminated.
