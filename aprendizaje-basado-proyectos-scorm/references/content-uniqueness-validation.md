...
Use this reference before writing or finalizing any package of three JSON files (one per ABPr learning moment). Passing the structural contract is not enough: every scenario/content and every thematic section must be academically unique, clearly aligned with learning outcomes, and free of repeated boilerplate.

## Diversity Blueprint

Before drafting text, create an internal 3 x 5 blueprint for the package. Do not include it in the JSON unless the app later supports it.

For each `contenido-01` through `contenido-03` (Diagnóstico Inicial, Análisis y Diseño, Conclusiones), and each `seccion-1` through `seccion-5`, define:
...
Generate the JSON only after the blueprint shows 15 distinct thematic sections. If two rows could swap titles and still make sense, they are too generic.
...
```bash
node aprendizaje-basado-proyectos-scorm/scripts/validate_content_package.js <slug-asignatura>-contenido-01.json <slug-asignatura>-contenido-02.json <slug-asignatura>-contenido-03.json
```
