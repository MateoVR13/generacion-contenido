...
## Fase 2 — Planeación didáctica (diseño inverso, ABPr — planeación por momentos)
- **Hacer:** diseñar la planeación virtual desde el RA y la configuración por créditos. Justificar ABPr.
  **ABPr usa 3 MOMENTOS FIJOS** (no un N variable de temas): siempre
  **`["Diagnóstico Inicial", "Análisis y Diseño", "Conclusiones"]`**. Planea cada momento con su tabla de
  diseño (Momento | RA | propósito del momento | preguntas orientadoras). Define el **escenario inicial ABPr**
  (problema real + pregunta direccionadora; trabajo colaborativo en equipos), la ruta de navegación, las
  actividades sincrónicas/asincrónicas, la distribución de horas, el documento y recursos proyectados y la
  secuencia evaluativa. No propongas ni discutas el número de momentos: son 3 fijos.
- **PROPONER LOS SUBTEMAS RELEVANTES por momento (solo listado, a validar).** Para cada uno de los 3 momentos,
  deriva del syllabus/saberes **los subtemas más relevantes que el contenido debería cubrir, únicamente
  listados, sin desarrollarlos** (3–7 por momento como referencia, no exhaustivos). Sirven para que el docente
  valide la **cobertura disciplinar** en el instrumento. No inventes subtemas que el syllabus no respalde; lo
  que no esté soportado, decláralo como brecha. Van a la sección 3 del instrumento docente (tabla aparte de
...
- **PROPONER EL MATERIAL COMPLEMENTARIO** (cantidad por créditos: 3/5/7) como una **matriz revisable**:
  ...
  base, libro complementario, recurso abierto, etc.), `idioma` (`es`/`en`), `momento(s) asociado(s)` y
  `justificacion`. Define la **cobertura por momento** (qué materiales cubren cada momento). Esta propuesta va
  al instrumento docente para su **revisión y aprobación** (no se incluye en los escenarios hasta que el
  profesor la valide).
...
- **Entregables (`fase-2-planeacion-didactica.md`):** configuración didáctica; justificación ABPr; RA eje;
  escenario inicial (problema + pregunta direccionadora); ruta del estudiante; tabla de los 3 momentos ABPr
  (PROPUESTA de propósitos/preguntas, a validar); distribución de horas; recursos proyectados; **matriz de
  material complementario propuesto + cobertura por momento (PROPUESTA, a validar)**.
- **Estado:** `momentos[]{n, titulo, ra, propositoEtapa, preguntasOrientadoras[], subtemas[]}` (3 elementos
  fijos: Diagnóstico Inicial, Análisis y Diseño, Conclusiones), `escenarioInicial{problema, preguntaDireccionadora}`,
  `rutaNavegacion`, `recursosProyectados`,
  `materialComplementario[]{id, apa, tipo, momentos[], justificacion, estado:"propuesto", href, todo}`,
  `coberturaPorMomento{momentoN:[ids]}`.

## Fase 3 — Diseño de evaluación
- **Hacer:** estrategia evaluativa alineada al RA y ABPr, cualitativa (`lineamientos-ua.md`). Mapa de
  evaluación; diagnóstica (10/15); formativas de seguimiento (**taller por escenario/momento**); sumativa
  integradora (socialización del proyecto, última semana); rúbricas analítica/holística; foro debate, wiki,
  coevaluación de equipo. Ponderación como brecha+propuesta si falta.
...
- **TALLERES por escenario — REDACTAR el propuesto por momento (va al instrumento).** Para cada momento,
  ...
  lo valida. Orienta cada taller al avance del proyecto del equipo (diagnóstico, diseño, socialización).
...
- **Entregables (`fase-3-diseno-evaluacion.md`):** mapa general de evaluación; **banco diagnóstico propuesto
  ...
  complementarios; momentos inicial/avance/final; auto/co/hetero (coevaluación de equipo).
