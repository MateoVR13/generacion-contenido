...
New asignatura generation produces three independent JSON files, one per ABPr learning moment (content NN develops moment NN):

- `<slug-asignatura>-contenido-01.json` — Diagnóstico Inicial
- `<slug-asignatura>-contenido-02.json` — Análisis y Diseño
- `<slug-asignatura>-contenido-03.json` — Conclusiones
...
"methodology": {
  "code": "ABPr",
  "name": "Aprendizaje Basado en Proyectos",
  "rationale": "Ruta que prepara al estudiante para diseñar y desarrollar un proyecto: diagnóstico de la problemática, análisis y diseño de la solución y socialización de conclusiones, con trabajo colaborativo en equipos.",
  "moments": ["Diagnóstico Inicial", "Análisis y Diseño", "Conclusiones"]
}
```
...
```json
{
  ...
  "methodologyMoment": "Diagnóstico Inicial",
...
For each of the three content JSON files, `scorm.sectionOrder` must contain exactly six ids:
...
For complete asignatura generation, the package contains 15 thematic sections: three contents times five thematic sections. These 15 sections must be unique in concept, explanation, example, activity, visual support, assessment, and RA evidence.

Before generating JSON, create an internal 3 x 5 diversity blueprint. Each row must define a distinct disciplinary concept, RA evidence, learner action, example/case, visual artifact, code or trace task when relevant, misconception, guided practice, and feedback. If two sections can exchange their titles without changing the body, the content fails the contract.
...
For each of the three content JSON files, `pdf.sectionOrder` must contain exactly six ids:

```json
["pdf-intro", "pdf-seccion-1", "pdf-seccion-2", "pdf-seccion-3", "pdf-seccion-4", "pdf-seccion-5"]
```

Each PDF uses the same minimum page extension requested by the user. Do not divide the requested pages across the three files.
...
"pdf": {
  ...
  "kicker": "Aprendizaje basado en proyectos · Práctica aplicada · Autoevaluación",
...
- New asignatura packages must produce three JSON files, each with six SCORM sections and six PDF sections.
...
- Full-package generation must pass a content-uniqueness validation before final delivery. When available, run `node aprendizaje-basado-proyectos-scorm/scripts/validate_content_package.js <generated-json-files>` and treat duplicate long strings or repeated 12-word sequences across thematic sections as blocking errors.
