...
actualizar `estado-proyecto.json`: ajustes a propósitos/preguntas/subtemas por momento (los 3 momentos son
fijos, no cambian), actividades del estudiante aprobadas, recursos (incluidos los de **simulación/laboratorio
si la asignatura es práctica**), sugerencias y feedback. A partir de aquí todo se genera con los valores
VALIDADOS.

---

## Fase 5 — Diseño tecnopedagógico y disciplinar (Documento de Saberes)
- **Hacer:** producir el Documento de Saberes ABPr (3 momentos fijos validados, extensión por créditos).
- **Salida OBLIGATORIA: los 3 archivos JSON finales** `<slug>-contenido-01.json … -03.json`
  (**ABPr = 3 momentos = 3 archivos**: `01` Diagnóstico Inicial, `02` Análisis y Diseño, `03` Conclusiones),
  conformes al contrato de la plantilla SCORM/PDF (`references/json-contract.md` de ESTA skill). Estos JSON son
  cargables directamente en la plantilla; **no basta con dejar un esquema en Markdown**. El esquema de
  contenido es solo un paso intermedio interno: la Fase 5 NO se considera cerrada hasta que existan los 3 JSON
  parseables y válidos.
- **Esta misma skill (`aprendizaje-basado-proyectos-scorm`) es el motor de contenido** — no delega a otra. Aplica
  el contrato y las reglas de componentes de SUS PROPIAS references (`references/json-contract.md`,
  `references/component-selection.md`, `references/instructional-content-requirements.md`,
  `references/content-uniqueness-validation.md`, `references/math-latex-validation.md`): 3 bloques teóricos por
  sección, carrusel/flashcards/acordión, steppers en LaTeX, charts con description+note, banner saberes-link,
  etc. Usa los parámetros validados (3 momentos fijos, propósito/preguntas orientadoras y subtemas por momento,
  RA, extensión por créditos, decisiones del docente) y `unitLabel: "Momento"`, `contentTotal: "03"`.
- Cada concepto central: definición, contexto teórico, principios, relevancia, aplicación en ABPr (avance del
  proyecto del equipo), ejemplos transversales (varias ingenierías/áreas), errores frecuentes, preguntas de
...
- **Entregables (todos obligatorios):**
  - `documento-saberes/<slug>-contenido-01.json … -03.json` (los 3 JSON de la plantilla SCORM/PDF).
...
- **Talleres por escenario:** cada momento lleva su taller (aprobado en P1) redactado completo dentro de su
  ...
  retroalimentación), orientado al avance del proyecto del equipo. La **sumativa integradora** (socialización
  del producto, todos los RA, última semana) en el último momento (Conclusiones) o
...
- **Reutiliza `moodle-content`**: genera el JSON de los componentes Moodle (bienvenida, página por momento,
  cierre) que el renderer convierte en HTML pegable. Son **3 momentos = 3 páginas** (Diagnóstico Inicial,
  Análisis y Diseño, Conclusiones). El archivo de Moodle es `<slug>-ABPr.moodle.json`.
- **Entregables:** JSON de moodle-content (`<slug>-ABPr.moodle.json`) + `fase-7-montaje-lms.md` (plan de aula,
...
- Fase 5 aplica las reglas de contenido de ESTA skill (no recargar lo que ya está en sus references); Fase 7
  delega a moodle-content; no reproducir su contenido en contexto.
