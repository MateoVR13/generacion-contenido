...
1. Convert the user request and any provided syllabus into a package of **three** reusable `subject` JSON files for SCORM/PDF, one per ABPr learning moment.
2. Ask for missing essentials only when they block generation: asignatura name, target level, language, syllabus source, and **minimum pages per PDF**. The page extension always applies to each of the three PDFs, not to the total package.
...
5. Use only the files inside this `aprendizaje-basado-proyectos-scorm/` folder as the skill's reusable reference base; do not depend on `../requisitos-contenido/` or external attachments unless the user provides new source material in the current task.
...
8. For new asignatura generation, write exactly three artifacts in the current project root using this naming pattern: `<slug-asignatura>-contenido-01.json` through `<slug-asignatura>-contenido-03.json`. Each file develops one ABPr moment: `contenido-01` = "Diagnóstico Inicial", `contenido-02` = "Análisis y Diseño", `contenido-03` = "Conclusiones". Do not create, overwrite, or replace `content.json` for generated asignatura content.
...
11. Use only **ABPr - Aprendizaje Basado en Proyectos** as methodology for generated virtual asignaturas. Do not use guided-learning, challenge, research, case, or any non-ABPr active methodology unless the app and institutional rules are explicitly changed later.
12. Each of the three JSON files must contain exactly six SCORM sections and exactly six PDF sections: one introductory section plus five content sections.
13. Before generating text, create an internal 3 x 5 diversity blueprint for the full package using `references/content-uniqueness-validation.md`. Each content and each thematic section must have a distinct concept, RA evidence, example/case, visual artifact, code or trace task, misconception, practice, and feedback. Do not include the blueprint in JSON unless the app later supports it.
...
16. Draft each thematic SCORM section (`seccion-1` through `seccion-5`) as a coherent learning sequence built on **exactly three theory blocks, and after each theory block place two or three expert-selected complementary components**: `theory-block` -> 2-3 components -> `theory-block` -> 2-3 components -> `theory-block` -> 2-3 components (practice/validation/closure). This produces a section with three theory-then-component cycles, enough conceptual development, examples/visualizations, guided practice, formative validation, and closure/transfer. Across the three component clusters of every thematic SCORM section, it is **mandatory** to include at least one `carousel`, at least one `flashcards`, and at least one `accordion` component; distribute them across the clusters where they best reinforce the preceding theory, never stacked together as filler.
...
18. Run a dedicated content-uniqueness validation pass before finalizing. If Node.js is available, run `node aprendizaje-basado-proyectos-scorm/scripts/validate_content_package.js <generated-json-files>`. Treat repeated learner-facing paragraphs, bullets, examples, exercises, feedback, code scaffolds, or visual prompts as blocking defects and rewrite them conceptually.
...
- Treat every asignatura as 100% virtual and generated as three content JSON files, one per ABPr learning moment. Do not generate a single all-course JSON for a new asignatura.
- Every generated JSON must represent one content number (`subject.contentNumber`: `01` to `03`) and should include `subject.contentTotal: "03"`. Each content number maps to its ABPr moment: `01` = "Diagnóstico Inicial", `02` = "Análisis y Diseño", `03` = "Conclusiones".
...
- Across the full package, the 15 thematic sections (`3 contenidos x 5 secciones`) must be conceptually and textually unique. Do not reuse paragraph shells, bullets, table rows, exercise prompts, expected answers, examples, code/pseudocode scaffolds, feedback, image prompts, or closing prompts with only the topic name changed.
...
- In every generated JSON, each thematic SCORM section (`seccion-1` through `seccion-5`) must contain **at least three** `theory-block` or `concept-block` components. Do not count `intro` toward this requirement. If the user explicitly requests a single-unit JSON instead of three files, apply the same rule to that unit's five thematic SCORM sections.
...
- Use `subject.methodology.code: "ABPr"`, `subject.methodology.name: "Aprendizaje Basado en Proyectos"`, and `subject.methodology.moments: ["Diagnóstico Inicial", "Análisis y Diseño", "Conclusiones"]` in every JSON.
...
- **Each numbered unit is called an "Escenario de aprendizaje", not "Contenido".** In every learner-facing label that names the numbered unit — section `badge`, `eyebrow`, intro text, `pdf.subtitle`, presentation key ideas, etc. — write "Escenario de aprendizaje NN" (e.g. "Escenario de aprendizaje 02", "Escenario de aprendizaje 03 de 03"), never "Contenido NN". The JSON data keys (`subject.contentNumber`, `subject.contentTotal`) and the file naming pattern (`<slug>-contenido-NN.json`) keep their names; only the visible wording changes.
- For methodology planning tables, routes, and `subject.methodology.moments`, use the official ABPr moment names as the labels. Write `Diagnóstico Inicial`, `Análisis y Diseño`, `Conclusiones`; do not write `Fase`, `Etapa`, or `Tema o etapa`. Content `01` develops "Diagnóstico Inicial", `02` "Análisis y Diseño", and `03` "Conclusiones".
- The asignatura presentation must introduce the discipline, purpose, scope, methodology, expected student work, and professional/academic value. Frame the content as the theoretical foundation that prepares the learner to design and develop a PROJECT: present the initial scenario as a real problem with a guiding question ("pregunta direccionadora"), and explain how the documento de saberes provides the conceptual bases to identify and implement transformation actions, analyze the problem, plan and design the solution, and socialize the project product (with emphasis on collaborative teamwork). Do not use the opening to list isolated technical topics or empty project slogans.
...
- Check new asignatura generation produced exactly three files named `<slug-asignatura>-contenido-01.json` through `<slug-asignatura>-contenido-03.json`, not `content.json`, and that each file develops its ABPr moment (`01` Diagnóstico Inicial, `02` Análisis y Diseño, `03` Conclusiones).
- Check every generated file has `subject.contentNumber`, `subject.contentTotal: "03"`, and consistent title/metadata for that content.
- Check every generated file has exactly six SCORM sections and exactly six PDF sections: intro plus five content sections.
- Check the 3 x 5 diversity blueprint was applied: every thematic section has a distinct disciplinary concept, RA evidence, example/case, visual artifact, code or trace task, misconception, practice, and feedback.
...
- Check methodology moments and tables use the official ABPr moment names (`Diagnóstico Inicial`, `Análisis y Diseño`, `Conclusiones`), never `Fase`, `Etapa`, or `Tema o etapa`.
- Check the only methodology is ABPr - Aprendizaje Basado en Proyectos; do not use any other methodology name, code, moments, labels, or structure.
...
- Run the content uniqueness validation pass from `references/content-uniqueness-validation.md`; when possible execute `node aprendizaje-basado-proyectos-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until it passes.
...
- Read `references/content-uniqueness-validation.md` before generating or revising a full three-file package, and before revising any PDF/SCORM content where repetition was detected.
