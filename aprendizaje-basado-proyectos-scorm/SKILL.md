...
- **Conexión SCORM → Documento de Saberes (PDF).** Cada tema SCORM debe incluir un componente de aviso `callout` (variante informativa, p. ej. `variant: "info"`, título "Puedes profundizar este tema en el Documento de Saberes") que le indique al estudiante que el desarrollo ampliado del tema está en el Documento de Saberes (PDF). Es solo informativo: **no** redirige ni lleva enlace funcional; refuerza la articulación SCORM↔PDF.
