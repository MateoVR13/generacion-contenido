#!/usr/bin/env node
"use strict";
// Valida el estado-proyecto.json contra los lineamientos UA para ABPr.
...
// ABPr: 3 momentos fijos.
const MOMENTOS_ABPR = ["Diagnóstico Inicial", "Análisis y Diseño", "Conclusiones"];

if (st.metodologia !== "ABPr") errors.push(`metodologia debe ser "ABPr", es "${st.metodologia}"`);
...
// ABPr: 3 MOMENTOS FIJOS (Diagnóstico Inicial, Análisis y Diseño, Conclusiones). No es N variable.
if (fase >= 2) {
  if (!Array.isArray(st.momentos)) {
    errors.push("momentos no definido (ABPr: deben planearse los 3 momentos fijos en F2)");
  } else if (st.momentos.length !== 3) {
    errors.push(`ABPr requiere exactamente 3 momentos fijos; momentos.length=${st.momentos.length}`);
  } else {
    st.momentos.forEach((m, i) => {
      if (m && m.titulo && m.titulo !== MOMENTOS_ABPR[i])
        warnings.push(`momento ${i + 1} titulo="${m.titulo}"; oficial ABPr = "${MOMENTOS_ABPR[i]}"`);
    });
  }
}
...
// Documento de Saberes (3 archivos, uno por momento) + JSON de Moodle. Verifica en disco junto al estado.
if (st.pipeline2Completo === true) {
  const dir = require("path").dirname(require("path").resolve(file));
  // (a) Documento de Saberes: 3 archivos <slug>-contenido-NN.json en documento-saberes/ (ABPr = 3 momentos = 3 archivos)
  let dsCount = 0;
  ...
  if (dsCount === 0)
    ...
    warnings.push(`Documento de Saberes: ${dsCount} JSON encontrados, se esperaban 3 (uno por momento ABPr).`);
  // (b) JSON de Moodle en fase-7-montaje-lms/ (<slug>-ABPr.moodle.json)
  ...
  if (!hasMoodle)
    errors.push("Pipeline 2 marcado completo pero NO hay JSON de Moodle (fase-7-montaje-lms/<slug>-ABPr.moodle.json).");
