---
name: aprendizaje-basado-retos-scorm
description: Virtualiza asignaturas 100% virtuales con metodología Aprendizaje Basado en Retos (ABR) siguiendo el proceso oficial UA de 11 fases (0–10) en 2 pipelines con gate de validación docente, Y genera los JSON de contenido SCORM/PDF. Úsala para (a) ejecutar el Pipeline 1 (Fases 0–4) que produce el INSTRUMENTO DE VALIDACIÓN DOCENTE a partir de un syllabus, y (b) ejecutar el Pipeline 2 (Fases 5–10) que, con el instrumento validado, genera el Documento de Saberes (los 4 JSON `subject`-based para la plantilla SCORM/PDF, uno por momento), los recursos e-learning y sus prompts, el montaje LMS y los entregables de QA/implementación/seguimiento. ABR usa 4 MOMENTOS FIJOS (Definición del reto, Ideación, Solución e implementación, Validación y evaluación), un archivo JSON por momento, extensión por créditos (30–55 págs), evaluación cualitativa, similitud ≤30% y APA 7. El escenario inicial plantea una situación-contexto, define el reto y formula preguntas direccionadoras. La Fase 5 aplica el contrato de componentes de esta misma skill: selección experta por disciplina, secuencia theory-first (≥3 theory-block por sección temática), carrusel/flashcards/acordeón obligatorios, steppers en matemáticas, Python para resolver ejercicios cuantitativos, charts con hilo conductor + nota, LaTeX validado, banner saberes-link, PDF editorial por facultad.
---

# Virtualización ABR + Contenido SCORM/PDF
...
3. **Lee solo lo de la fase en curso** (token-eficiencia): siempre `references/lineamientos-ua.md` + `references/proceso-fases-ua.md` + `references/metodologia-abr.md`; Pipeline 1 → `references/pipeline-1-fases-0-4.md` + `references/instrumento-docente.md`; Pipeline 2 → `references/pipeline-2-fases-5-10.md` + `references/pipeline-documento-saberes.md` + `references/fase-6-prompts.md`.
...
7. **Pipeline 2 arranca** con el instrumento respondido: ingiere las decisiones del docente al estado y genera todo. La **Fase 5** produce los **4 JSON del Documento de Saberes** (uno por momento ABR) siguiendo el **Core Workflow** y las reglas de contenido de abajo (esta misma skill es el motor de contenido — no delega a otra). La **Fase 7** usa `moodle-content`.
8. **Cierre del Pipeline 2:** deben existir y validar los 4 JSON `<slug>-contenido-NN.json` (Documento de Saberes, uno por momento) y el `<slug>-ABR.moodle.json` (Moodle).

# SCORM/PDF JSON Content (Fase 5 — motor de contenido)

## Core Workflow

1. Convert the user request and any provided syllabus into a package of **four** reusable `subject` JSON files for SCORM/PDF, one per learning moment of Aprendizaje Basado en Retos.
...
5. Use only the files inside this `aprendizaje-basado-retos-scorm/` folder as the skill's reusable reference base; do not depend on `../requisitos-contenido/` or external attachments unless the user provides new source material in the current task.
...
8. For new asignatura generation, write exactly four artifacts in the current project root using this naming pattern: `<slug-asignatura>-contenido-01.json` through `<slug-asignatura>-contenido-04.json`. Each file NN develops the learning moment NN of Aprendizaje Basado en Retos (01 = Definición del reto, 02 = Ideación, 03 = Solución e implementación, 04 = Validación y evaluación). Do not create, overwrite, or replace `content.json` for generated asignatura content.
...
11. Use only **ABR - Aprendizaje Basado en Retos** as methodology for generated virtual asignaturas. Do not use guided, project, research, case, or any non-ABR active methodology unless the app and institutional rules are explicitly changed later.
12. Each of the four JSON files must contain exactly six SCORM sections and exactly six PDF sections: one introductory section plus five content sections.
...
18. Run a dedicated content-uniqueness validation pass before finalizing. If Node.js is available, run `node aprendizaje-basado-retos-scorm/scripts/validate_content_package.js <generated-json-files>`. Treat repeated learner-facing paragraphs, bullets, examples, exercises, feedback, code scaffolds, or visual prompts as blocking defects and rewrite them conceptually.
...
- Every generated JSON must represent one content number (`subject.contentNumber`: `01` to `04`) and should include `subject.contentTotal: "04"`. Content NN develops moment NN (01 = Definición del reto, 02 = Ideación, 03 = Solución e implementación, 04 = Validación y evaluación).
...
- Use `subject.methodology.code: "ABR"`, `subject.methodology.name: "Aprendizaje Basado en Retos"`, and `subject.methodology.moments: ["Definición del reto", "Ideación", "Solución e implementación", "Validación y evaluación"]` in every JSON.
...
- For methodology planning tables, routes, and `subject.methodology.moments`, use the official Aprendizaje Basado en Retos moment names as the labels. Write `Definición del reto`, `Ideación`, `Solución e implementación`, `Validación y evaluación`; do not write `Fase`, `Etapa`, or `Tema o etapa`. Content NN corresponds to moment NN in that exact order.
...
- Check every generated file has `subject.contentNumber`, `subject.contentTotal: "04"`, and consistent title/metadata for that content, and that content NN develops moment NN of Aprendizaje Basado en Retos.
...
- Check methodology moments and tables use the official Aprendizaje Basado en Retos moment names (`Definición del reto`, `Ideación`, `Solución e implementación`, `Validación y evaluación`), never `Fase`, `Etapa`, or `Tema o etapa`.
- Check the only methodology is ABR - Aprendizaje Basado en Retos; do not use any other methodology name, code, moments, labels, or structure.
...
- Run the content uniqueness validation pass from `references/content-uniqueness-validation.md`; when possible execute `node aprendizaje-basado-retos-scorm/scripts/validate_content_package.js <generated-json-files>` and revise until it passes.
