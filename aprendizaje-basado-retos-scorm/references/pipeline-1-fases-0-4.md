...
> Nota de nomenclatura: para no chocar con los **momentos** ABR (que también podrían leerse como
> "etapas"), en prosa esta metodología puede referir las fases institucionales como **"Etapa 0–4"**; pero
> los nombres de fase del pipeline (**F0–F10**) se mantienen tal cual.
...
## Fase 2 — Planeación didáctica (diseño inverso, ABR por momentos)
- **Hacer:** diseñar la planeación virtual desde el RA y la configuración por créditos. Justificar ABR.
  **ABR NO propone un N variable de temas: usa los 4 momentos fijos** de Aprendizaje Basado en Retos
  (`["Definición del reto", "Ideación", "Solución e implementación", "Validación y evaluación"]`). Construye
  la **tabla de diseño por momento** (Momento | RA | propósito | preguntas direccionadoras). Define el
  **escenario inicial** (situación-contexto + reto definido + preguntas direccionadoras), la ruta de
  navegación, actividades sincrónicas/asincrónicas, distribución de horas, documento y recursos proyectados,
  secuencia evaluativa.
- **PROPONER LOS SUBTEMAS RELEVANTES por momento (solo listado, a validar).** Para cada momento, deriva del
  syllabus/saberes **los subtemas más relevantes que el contenido debería cubrir, únicamente listados, sin
  desarrollarlos** (3–7 por momento como referencia, no exhaustivos). Sirven para que el docente valide la
...
- **PROPONER EL MATERIAL COMPLEMENTARIO** (cantidad por créditos: 3/5/7) como una **matriz revisable**:
  ...
  al instrumento docente para su **revisión y aprobación** (no se incluye en los momentos hasta que el
  profesor la valide).
...
- **Entregables (`fase-2-planeacion-didactica.md`):** configuración didáctica; justificación ABR; RA eje;
  escenario inicial (situación-contexto + reto + preguntas direccionadoras); ruta del estudiante; tabla de
  los 4 momentos ABR; distribución de horas; recursos proyectados; **matriz de material complementario
  propuesto + cobertura por momento (PROPUESTA, a validar)**.
- **Estado:** `momentos[]{titulo, ra, propositoEtapa, preguntasOrientadoras[], subtemas[]}` (los 4 momentos
  fijos), `escenarioInicial{situacion, reto, preguntasDireccionadoras[]}`, `rutaNavegacion`,
  `recursosProyectados`,
...
- **Hacer:** estrategia evaluativa alineada al RA y ABR, cualitativa (`lineamientos-ua.md`). Mapa de
  evaluación; diagnóstica (10/15); formativas de seguimiento (**taller por momento**); sumativa
...
- **TALLERES por momento — REDACTAR el propuesto por momento (va al instrumento).** Para cada momento, propón
  el taller (enunciado, evidencia, RA, criterio, instrucciones) en la tabla de la sección 4; el docente lo
...
- **Entregables (`fase-3-diseno-evaluacion.md`):** mapa general de evaluación; **banco diagnóstico propuesto
  (15 preguntas)**; **talleres por momento propuestos**; formativas; sumativa; rúbricas; instrumentos
